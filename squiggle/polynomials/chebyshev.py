...
from squiggle.polynomials import polyutils as pu
...
def chebfromroots(roots):
    if len(roots) == 0:
        ...
        def p_scan_fun(carry, x):
            return carry, _chebadd(
                jnp.zeros(retlen, dtype=x.dtype), jnp.array([-x, 1], dtype=x.dtype)
            )
        ...
        def body_fun(val):
            ...
            tmp = lax.cond(
                r,
                lambda x: x.at[0].set(_chebmul(x[0], arr[2 * m])[:retlen]),
                lambda x: x,
                tmp,
            )
...
def chebder(c, m=1):
    ...
    if cnt >= n:
        ...
        def outer_body_fun(i, outer_val):
            ...
            tmp, outer_val = lax.fori_loop(
                -inner_start, -2, inner_body_fun, (tmp, outer_val)
            )

            tmp = lax.cond(
                inner_start > 1,
                lambda x: x.at[1].set(4 * outer_val[2]),
                lambda x: x,
                tmp,
            )
...
def _chebfit(x, y, deg, rcond=None, full=False, w=None, numpy_resid=True):
    ...
    if issubclass(lhs.dtype.type, jnp.complexfloating):
        scl = jnp.sqrt((jnp.square(lhs.real) + jnp.square(lhs.imag)).sum(1)).astype(
            lhs.dtype
        )
    else:
...
def chebfit(x, y, deg, rcond=None, full=False, w=None, numpy_resid=True):
    if numpy_resid:
        ...
        return jit(
            partial(_chebfit, numpy_resid=False), static_argnames=("deg", "full")
        )(x, y, deg, rcond=rcond, full=full, w=w)
...
def _chebcompanion(c):
    ...
    scl = jnp.array([1.0] + [jnp.sqrt(0.5)] * (n - 1), dtype=c.dtype)
...
def chebgauss(deg):
    ...
    x = jnp.cos(jnp.pi * jnp.arange(1.0, 2 * deg, 2.0) / (2.0 * deg))
